...
class ShipmentControllerShipmentsTest {

    private final ShipmentPackageService shipmentPackageService = mock(ShipmentPackageService.class);

    private MockMvc mockMvc;

    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(new ShipmentController(mock(ShippingService.class), shipmentPackageService))
        ...
        ShipmentResponse response = new ShipmentResponse(50L, 3L, "Ada Lovelace", ShipmentStatus.CREATED,
                "Musterstr. 1", "DHL", null, LocalDate.of(2026, 10, 1), null, null, null, 0, BigDecimal.ZERO, List.of());
        when(shipmentPackageService.createShipment(any())).thenReturn(response);
        when(shipmentPackageService.addShipmentPackages(any(), any())).thenReturn(response);
        when(shipmentPackageService.replaceShipmentPackages(any(), any())).thenReturn(response);
        when(shipmentPackageService.removeShipmentPackage(any(), any())).thenReturn(response);
        when(shipmentPackageService.updateShipmentData(any(), any())).thenReturn(response);
        when(shipmentPackageService.findShipments(any(), any()))
    ...
    void createsAShipmentWith201() throws Exception {
        ...
        verify(shipmentPackageService).createShipment(argThat((CreateShipmentRequest request) ->
    ...
    void refusesACreateRequestWithoutCustomerPackagesOrAddress() throws Exception {
        ...
        verifyNoInteractions(shipmentPackageService);
    }
    ...
    void addsPackagesFromABareArray() throws Exception {
        ...
        verify(shipmentPackageService).addShipmentPackages(eq(50L),
    ...
    void replacesPackagesFromTheWrappedForm() throws Exception {
        ...
        verify(shipmentPackageService).replaceShipmentPackages(eq(50L),
    ...
    void removesAPackageByTheIdsInThePath() throws Exception {
        ...
        verify(shipmentPackageService).removeShipmentPackage(50L, 7L);
    }
    ...
    void listsWithoutAStatusByDefault() throws Exception {
        ...
        verify(shipmentPackageService).findShipments(isNull(), any(Pageable.class));
    }
    ...
    void answersAConflictFromTheServiceWith409() throws Exception {
        when(shipmentPackageService.createShipment(any())).thenThrow(
